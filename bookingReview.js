...
function initializeConfirmation() {
    ...
    params.vehicleFeatures.split(',').forEach(feature => {
    ...
    document.getElementById('return-datetime').textContent =
        params.returnDate
...
document.getElementById('pay-now-btn').addEventListener('click', () => {
    ...
    const passengers = parseInt(params.passengers, 10) || 1;
    let priceToCharge = total;

    // If more than 1 passenger and user clicks Pay Now,
    // ask if they want to pay the entire total or create a group
    if (passengers > 1) {
        const okFull = confirm(
            `You selected ${passengers} passengers.\n\n` +
            `Do you want to pay the full amount of $${total.toFixed(2)} yourself?\n\n` +
            `Press OK to pay the full amount, or Cancel to create a group trip and split the cost.`
        );

        if (!okFull) {
            // redirect to create group trip instead
            const url = 'createTrip.html' +
                '?vehicle=' + encodeURIComponent(params.vehicleName) +
                '&type=' + encodeURIComponent(params.vehicleType) +
                '&seats=' + encodeURIComponent(params.vehicleSeats) +
    ...
    const url = 'payment.html' +
        '?vehicle=' + encodeURIComponent(params.vehicleName) +
        '&price=' + encodeURIComponent(priceToCharge.toFixed(2)) +
        '&passengers=' + encodeURIComponent(passengers) +
        '&pickup=' + encodeURIComponent(params.departureAddress) +
        '&dropoff=' + encodeURIComponent(params.returnAddress) +
        '&departure-date=' + encodeURIComponent(params.departureDate) +
        '&return-date=' + encodeURIComponent(params.returnDate) +
        '&departure-time=' + encodeURIComponent(params.departureTime) +
        '&return-time=' + encodeURIComponent(params.returnTime) +
